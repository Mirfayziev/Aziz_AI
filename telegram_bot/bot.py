...
async def telegram_webhook(req: Request):
    try:
        ...
        # ===== /start =====
        if msg.get("text", "").lower() in ("/start", "start"):
            await tg_send_text(
                chat_id,
                "Assalomu alaykum.\n"
        ...
        if "voice" in msg:
            ...
            payload["voice_base64"] = voice_bytes.hex()

        async with httpx.AsyncClient(timeout=180) as client:
            r = await client.post(
                f"{BACKEND_URL}/assistant-message",
                json=payload,   # 🔴 MUHIM: faqat JSON
            )
            r.raise_for_status()
            data = r.json()
        ...
        if data.get("audio_hex"):
            await tg_send_voice(chat_id, bytes.fromhex(data["audio_hex"]))

        return {"ok": True}

    except Exception as e:
        log.exception("Telegram webhook error: %s", e)
        # MUHIM: Telegram 500 ko‘rmasligi shart emas
        return {"ok": True}
